...
let passwordDivShow = document.querySelector('#createAccount')
let shipDivShow = document.querySelector('#ship')

passwordDivShow.addEventListener('change', () => {
    document.querySelector('.pass-checkout').classList.toggle('active')
})
shipDivShow.addEventListener('change', () => {
    document.querySelector('.billing-address-form').classList.toggle('active')
})
...
function userBasketIsFull(userBasket) {
    ...
    shopCartContentEmpty.style.display = 'none'
    shopCartSummery.style.display = 'block'
    setLocalStorage(userBasket)
    userBasketLangth(userBasket)
    calcTotalPrice(userBasket)
    basketProductsGenerator(userBasket)
...
function calcTotalPrice(userBasket) {
    ...
    priceTotalWithoutTax.innerHTML = '$' + (totalPriceValue).toFixed(2)
    totalShopCart.innerHTML = '$' + (totalPriceValue + 7.00).toFixed(2)
